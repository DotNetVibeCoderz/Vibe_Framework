...
//!
//! This is one of the project's load-bearing byte-level contracts, and it is
//! implemented twice — here and in `dotnet/tools/RustNet.Deploy/Rndp.cs`. The
//! two must stay in step; `docs/protocol.md` is the specification.
//!
//! The crate is `no_std + alloc` so bare-metal firmware speaks exactly the
//! same framing as the host-side virtual device, rather than a second
//! hand-rolled copy that can drift.

// `no_std` on the device; the test harness needs std to link.
