...
/// <summary>
/// AvaloniaEdit ships syntax colours tuned for a white page — navy tags and
...
public static class CodeEditorTheme
{
    ...
    /// AvaloniaEdit ships no Rust definition, so Rust borrows C++ — close enough
    /// for keywords, strings and comments.
    /// </summary>
    public static IHighlightingDefinition? DefinitionFor(string language)
    {
        ApplyDarkPalette();
        string name = RustNet.Designer.Assistant.CodeHighlighter.Normalize(language) switch
        {
            "csharp" => "C#",
            "xml" => "XML",
            "json" => "Json",
